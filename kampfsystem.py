        ...
        if faehigkeit.ziel_anzahl == "alle":
            ...
            for g in gegner_liste:
                if trifft_angriff(spieler, g) == True:
                    if g.current_HP > 0:
                        schaden_reduktion = (g.defense / (g.defense + 100))
            ...
            if trifft_angriff(spieler, ziel) == True:
                schaden_reduktion = (ziel.defense / (ziel.defense + 100))
...
def kampf(spieler, gegner_liste):
    ...
    while spieler.current_HP > 0 and any(g.current_HP > 0 for g in gegner_liste):
        ...
        if not gegner_liste:
            ...
            gold = sum(random.randint(10, 30) for _ in gegner_liste)
            exp = sum(g.exp_give for g in gegner_liste)
            spieler.gold += gold
            spieler.exp += exp
            print(f"Du erhältst {gold} Gold und {exp} EP!")
            return
